...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (eventType === "payment.captured") {
      ...
      if (payment && payment.order_id) {
        ...
        if (order) {
          ...
          if (order.userId) {
            const profile = await prisma.profile.findUnique({ where: { id: order.userId } });
            if (profile?.email) {
              ...
              try {
                await sendEmail({ to: profile.email, subject: "Your payment receipt", html });
                await prisma.invoice.update({ where: { id: invoice.id }, data: { sent: true } });
              } catch (e) {
      ...
      if (sub) {
        const dbSub = await prisma.subscription.findUnique({ where: { razorpaySubscriptionId: sub.id } });
        if (dbSub) {
    ...
    await prisma.webhookEvent.update({ where: { id: saved.id }, data: { processed: true, attempts: { increment: 1 } } });
