...
export function QrCreateForm({ workspaceId, folders, tags }: Props) {
  ...
  const handleSubmit = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (currentStep !== 'branding') {
      console.warn(
        'Form submit blocked: User is not on the final branding step.'
      );
      return;
    }

    console.log(
      'Form submission triggered by element:',
    ...
    try {
      ...
      const requiredFields = typeDef.fields.filter((field) => field.required);
